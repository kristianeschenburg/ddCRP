...
sys.path.append(''.join([codedir,'surface_utilities/surface_utilities/']))
sys.path.append(''.join([codedir,'ChineseRestaurantProcess/crp/']))
sys.path.append(''.join([codedir,'Parcellating-connectivity/python/model_similarity/']))

import adjacency as adj
import WardClustering

from ddcrp import ddCRP
from subgraphs import ClusterSpanningTrees

parser = argparse.ArgumentParser()
parser.add_argument('-surf','--surfacefile',help='Surface to compute adjacency on.',
    required=True,type=str)
parser.add_argument('-lab','--labelfile',help='Label map to get indices from.',
    required=True,type=str)
parser.add_argument('-data','--datafile',help='Data to cluster.',
    required=True,type=str)

parser.add_argument('-a','--alpha',help='Dirichlet process concentration parameter.',
...
parser.add_argument('-w','--ward',help='Booleant to run Ward clustering before ddCRP.',
    required=True,default=False,type=bool)
parser.add_argument('-mcmc','--monte',help='Number of MCMC iterations to run.',
    required=False,type=int,default=75);
...
adj_list = {inds2sort[k]: [inds2sort[v] for v in filtered[k]] for k in filtered.keys()}

print('Preparing features.')
features = niio.load(args.datafile)
[n,p] = features.shape
if p > n:
    features = features.T
...
crp = ddCRP(alpha,mu,kappa,nu,sigma,mcmc_passes=args.monte,stats_interval=200)
...
niio.save(full_mapz,''.join([outbase,'map_z.func.gii']),'CortexLeft')
